    ...
    public void run() {
        ...
        while (true) { // loop indefinately
            ...
            for (Line genericEventData : ReadScheduleFile.getScheduledEventData()) { // loop through all arraylist indexes
                if ((!AuthenticationDialogue.getMainProgramInstance().isHalted()) && (currentTimeInSeconds == genericEventData.getStartTime()) && ((genericEventData.getDate().equalsIgnoreCase("EVERYDAY") || genericEventData.getDate().equals(currentDate) || genericEventData.getDate().equals(month + "/" + dayOfMonth + "/YEARLY")))) { // if the time and date is the same as listed in the file or if file is set to yearly or everyday (yearly is used for holidays) and the program is not stopped, execute actions below
